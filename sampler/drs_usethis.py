...
class DRS(nn.Module):
    ...
    # -------------------------
    # -------------------------
    # Moreau-envelope likelihood gradient helpers (for prox-guided Langevin)
    # -------------------------
    @staticmethod
    ...
    def _moreau_grad_x(self, operator, x_m11: torch.Tensor, measurement: torch.Tensor,
        ...
        if mode == "phase_retrieval":
            ...
            # K^* grad_u in x01, then chain rule dx01/dx = 1/2
    ...
    def optimize_denoising(self, z_in, model, d_k, sigma,
        ...
        with torch.no_grad():
            ...
            # - 'legacy' keeps the previous heuristic drift toward forward_z.
            # - 'prox_likelihood' uses:  s_hat(x)=s_theta(x,σ) - K^T ∇ e_γ f_y(Kx)
            ...
            for _ in range(num_steps):
                ...
                if corrector_type == "prox_likelihood":
                    ...
                    lgvd_z = lgvd_z + noise_scale * (2 * lr) ** 0.5 * torch.randn_like(noisy_im)
                else:
                    ...
                    lgvd_z = lgvd_z + lr * score_val + drift + noise_scale * (2 * lr) ** 0.5 * torch.randn_like(noisy_im)
